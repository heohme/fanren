const EP_PATTERNS = [
  ...
  /第\s*0*(\d{1,4})\s*[集话回]/,
  // UP 主常在集数和“解析”之间插入主题词，如“180集台词细节解析”。
  /\b0*(\d{2,4})\s*[集话回]\s*[^0-9]{0,12}?(?:解析|解读|逐帧|漫谈|赏析|复盘|点评|细节分析|改编分析|预告)/,
  /(?:^|[^0-9])(?:EP|Ep|ep)\s*0*(\d{1,4})(?!\d)/,
  /(?:^|[^a-zA-Z0-9])E0*(\d{1,4})(?![a-zA-Z0-9])/,
  /[【\[]\s*0*(\d{1,4})\s*[】\]]/,
  // 兼容“《凡人修仙传》182逐帧解析”等带书名号、但省略“集”的写法。
  /(?:凡人修仙传|凡人)[》】\]）)]?\s*[-－—：:]?\s*0*(\d{2,3})(?=\s*(?:集|话|解析|解读|逐帧|漫谈|赏析|复盘|点评|预告|$))/,
];
